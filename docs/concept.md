...
- Credentials per stack: the consumer stack needs two accounts, so it cannot run from one set of pipeline credentials. Add `assume_role` per provider, or split it into two jobs